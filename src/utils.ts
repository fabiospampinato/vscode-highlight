...
import type {Decoration, Highlight} from './types';

/* MAIN */

const getDecoration = memoize ( ( regex: RegExp, value: vscode.DecorationRenderOptions ): Decoration => { // It's important to memoize decorations by regex too, it makes updating them simpler

  const valueSerialized = JSON.stringify ( value );
  const valueIsDynamic = /\$\d+/.test ( valueSerialized );

  if ( valueIsDynamic ) { // Dynamic decoration

    return ( match: RegExpExecArray ): vscode.TextEditorDecorationType => {

      const valueSerializedResolved = valueSerialized.replace ( /\$(\d)/g, ( _, index ) => match[index] );
      const value = JSON.parse ( valueSerializedResolved );
    ...
    const decoration = vscode.window.createTextEditorDecorationType ( value );
...
const getHighlights = (): Highlight[] => { //TODO: maybe cache this until the config changes... (measure performance here)
