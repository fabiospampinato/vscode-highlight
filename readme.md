...
4. Nested [non-capturing groups](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Regular_expressions/Non-capturing_group) are supported but nested _capturing_ groups are not. For example, `(\"(label|name)\")` is _not_ allowed because it is a capturing group nested within another capturing group. However, `(\"(?:label|name)\")` _is_ allowed because because `(?:label|name)` is a non-capturing group.
